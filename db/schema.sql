...
CREATE TABLE IF NOT EXISTS `kecamatan` (
  ...
  `kabupaten` varchar(100) NOT NULL DEFAULT 'Luwu Timur',
...
CREATE TABLE IF NOT EXISTS `petugas` (
  ...
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  `password_changed_at` datetime DEFAULT NULL,
  `harus_ganti_sandi` tinyint(1) NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
  UNIQUE KEY `nip` (`nip`),
  UNIQUE KEY `email` (`email`),
...
CREATE TABLE IF NOT EXISTS `bookings` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `nomor_berkas` varchar(100) NOT NULL,
  ...
  `koordinat_maps` varchar(255) DEFAULT NULL,
  `no_telepon` varchar(20) NOT NULL,
  `tanggal_diminta` date NOT NULL,
  `tanggal_fix` date DEFAULT NULL,
  `reschedule_count` tinyint(4) NOT NULL DEFAULT 0,
  `status` enum('pending','rescheduled_by_user','rescheduled_by_petugas','jadwal_fix','selesai','ditolak','dibatalkan') NOT NULL DEFAULT 'pending',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `hasil_pemeriksaan` (
  ...
  `foto_risalah` varchar(255) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `reschedule_log` (
  ...
  `diminta_oleh` enum('user','petugas') NOT NULL,
...
CREATE TABLE IF NOT EXISTS `otp_tokens` (
  ...
  `pending_registration_id` int(11) DEFAULT NULL,
  `email` varchar(150) NOT NULL,
  ...
  KEY `idx_otp_petugas` (`petugas_id`,`purpose`,`created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `pending_registrations` (
  ...
  `email_verified_at` datetime DEFAULT NULL,
  `status` enum('pending_email_verification','pending_profile_completion') NOT NULL DEFAULT 'pending_email_verification',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `kuota_default` (
  ...
  `kuota_max` int(11) NOT NULL DEFAULT 10,
  `is_unlimited` tinyint(1) NOT NULL DEFAULT 0,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  `set_order` bigint(20) NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `kuota_kecamatan` (
  ...
  `is_unlimited` tinyint(1) NOT NULL DEFAULT 0,
  `source` varchar(20) NOT NULL DEFAULT 'tanggal',
  `set_order` bigint(20) NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `audit_log` (
  ...
  `pelaku_peran` varchar(20) DEFAULT NULL,
  `pelaku_id` int(11) DEFAULT NULL,
  `pelaku_nama` varchar(150) DEFAULT NULL,
  `sasaran_jenis` varchar(30) DEFAULT NULL,
